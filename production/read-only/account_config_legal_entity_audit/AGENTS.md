...
**Env: production (any namespace).** Walks every `account_configuration`, follows each of
its plugins to the legal entity that plugin points at, and reports — field by field —
...
| `--country XX` | only this country; repeatable |
| `--provider ID` | only this provider_id; repeatable or comma-separated |
| `--conflicts-only` | hide rows that compared cleanly (terminal view only — the Markdown and CSV always carry every row) |
| `--include-prefix` | count `PREFIX` as a conflict |
...
Run with no flags and it prompts for the namespace, then for read approval. In production
a bare Enter at the approval prompt **cancels**.
