...
| [plugin_legal_entity_updates](production/write/plugin_legal_entity_updates/) | `./plugin_legal_entity_updates.js` (fully interactive) | Link plugins to their **primary** legal entity. Prompts for env, providers (all, or a list), and dry-run-vs-apply, then runs `link_plugins_to_legal_entities_from_env`. Reads `provider_purchases_primary_legal_entities` (shedul, `valid_to IS NULL` — same SQL as the `get_primary_legal_entity_id_for_provider` RPC) + `account_configuration_plugins`. Verifies by reading the rows back afterwards (per-plugin table + cross-check commands; exits 1 if a row didn't land) and prints an exempt-providers roster. Dry run by default; prod needs two confirmations. |
