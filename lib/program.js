...
OPS_ORION_SCRIPTS: absolute path of the Orion scripts repo. Takes precedence over
\`ops config get orion.scripts\` and over the default of a sibling checkout
(../orion/scripts next to the ops repo).

OPS_CONFIG_DIR: directory holding config.json. Default $XDG_CONFIG_HOME/ops or
~/.config/ops.

The scripts themselves read their own secrets from the repo-root .env of the
scripts repo (see .env.example there). ops never reads or forwards those; it only
reports blank ones in \`ops orion doctor\`.`,
  },
  "exit-codes": {
...
own: the gates belong to the scripts.`,
  ...
  conventions: {
    ...
    text: `Layout      <env>/<access>/<name>/ with env in {production, staging} and access
            in {read-only, write}. One directory per script, one executable
            entrypoint named after the directory (.js with #!/usr/bin/env node,
...
Docs        The script's --help owns its flags. AGENTS.md frontmatter owns the
            summary, tier, aliases and examples. Root catalogues are generated by
...
function addConfigCommands(program) {
  const cfg = program.command("config").summary("Manage configuration for ops").description(`Read and write ${config.CONFIG_FILE}.\n\nKeys are dotted paths. Known keys:\n  orion.scripts   absolute path of the Orion scripts repo`).helpGroup("CORE COMMANDS");
  gh(cfg, { examples: [{ cmd: "ops config set orion.scripts ~/Desktop/repos/orion/scripts" }, { cmd: "ops config get orion.scripts" }, { cmd: "ops config list" }] });
