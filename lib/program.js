...
secret that way — export it in your shell instead.
...
yellow for the staging wipe, cyan for the sandbox targets, green for read-only. A
...
function buildProgram() {
  ...
  gh(program, {
    ...
    examples: [
      { cmd: "ops run retry_invoices --dry-run 123,456", note: "run a script; the run is recorded" },
...
function addAliasCommands(program) {
  const alias = program
    ...
    .description("Aliases expand the first word of an ops invocation into several: `ops alias set tl 'task ls'` makes `ops tl --status failed` run `ops task ls --status failed`. An alias can never shadow a real command or topic.")
    .helpGroup("CORE COMMANDS");
  gh(alias, { examples: [{ cmd: "ops alias set tl 'task ls'" }, { cmd: "ops alias set sl 'orion script list'" }, { cmd: "ops alias list" }] });
