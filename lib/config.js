...
//   { "orion": { "scripts": "/abs/path" }, "aliases": { "osr": "orion script run" } }
...
// gh-style prefix expansion: `ops osr ri` -> `ops orion script run ri`.
// Only the first token is looked up, and never one that names a real command.
function expandAlias(argv, reserved) {
  if (argv.length === 0) return argv;
  const [head, ...rest] = argv;
  if (reserved.has(head)) return argv;
  const exp = aliases()[head];
  if (typeof exp !== "string") return argv;
  return [...exp.split(/\s+/).filter(Boolean), ...rest];
}

module.exports = { CONFIG_FILE, load, save, get, set, unset, entries, aliases, expandAlias };
