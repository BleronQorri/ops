...
// The Orion scripts catalogue: finds the scripts repo, walks
// <env>/<access>/<name>/, parses each AGENTS.md frontmatter, validates it and
// resolves names / aliases / secondary entrypoints to something runnable.
...
function scriptsRoot() {
  const candidates = [
    ...
    ["sibling checkout", path.resolve(__dirname, "..", "..", "..", "orion", "scripts")],
  ];
  for (const [, p] of candidates) {
    if (p && fs.existsSync(path.join(p, "production"))) return path.resolve(p);
  }
  throw new UsageError(
    "cannot find the Orion scripts repo",
...
function walkScriptDirs(root) {
  ...
  for (const env of ENVS) {
    for (const access of ACCESSES) {
      const base = path.join(root, env, access);
      if (!fs.existsSync(base)) continue;
      for (const ent of fs.readdirSync(base, { withFileTypes: true })) {
        if (!ent.isDirectory() || ent.name.startsWith(".")) continue;
        const dir = path.join(base, ent.name);
        const files = fs.readdirSync(dir);
        if (files.includes("AGENTS.md") || files.some((f) => isExecutable(path.join(dir, f)))) out.push({ env, access, name: ent.name, dir, rel: path.join(env, access, ent.name) });
      }
    }
  }
...
const KNOWN_KEYS = new Set(["name", "summary", "env", "access", "tier", "status", "lang", "entrypoint", "also", "aliases", "help_flag", "examples", "env_vars", "reports", "related", "blocked_on"]);
...
function validateScript(s, ctx) {
  ...
  else if (s.env !== s.rel.split(path.sep)[0]) err(`frontmatter env "${s.env}" != path "${s.rel.split(path.sep)[0]}"`);
  if (!ACCESSES.includes(s.access)) err(`frontmatter: access must be one of ${ACCESSES.join(", ")}`);
  else if (s.access !== s.rel.split(path.sep)[1]) err(`frontmatter access "${s.access}" != path "${s.rel.split(path.sep)[1]}"`);
  ...
  for (const v of s.env_vars) if (ctx.envKeys && !ctx.envKeys.has(v)) err(`env_vars: ${v} is not a key in .env.example`);
...
function loadCatalogue({ reserved = new Set() } = {}) {
  ...
  const ctx = { reserved, names, envKeys: envExampleKeys(root) };
