...
// The schemes we know by name, and the order their subsections appear in. The
// frontmatter is not closed to this list — a new country's scheme is a slug the
// day someone writes the first script for it — so one we have never heard of is
// title-cased from its slug and sorted after the ones we have, never dropped.
const INTEGRATIONS = {
  verifactu: { label: "Verifactu", blurb: "Spain: invoices registered with the AEAT as they are issued" },
  zatca: { label: "ZATCA", blurb: "Saudi Arabia: invoices cleared or reported to ZATCA (Fatoora)" },
  smart_receipts: { label: "Smart Receipts", blurb: "Italy: corrispettivi telematici lodged with the Agenzia delle Entrate" },
};
const INTEGRATION_IDS = Object.keys(INTEGRATIONS);

// Where a script goes when it names no scheme and no integrator. Unlike an
// unclassified domain this is not a gap to fix: a payload editor or a retry
// forcer works whatever scheme the document is bound for, and saying so by
// leaving the field out is the truth, not an omission.
const ANY_INTEGRATION = "any";
const SCHEME = "scheme:";
const INTEGRATOR = "integrator:";

// Which subsection a script belongs under: its scheme if it declares one; failing
// that whoever we reach the scheme through, which still narrows it to one
// partner's API; failing both, the tools that serve them all.
function integrationOf(s) {
  if (s.integration) return SCHEME + s.integration;
  if (s.integrator) return INTEGRATOR + s.integrator;
  return ANY_INTEGRATION;
}
...
// The integrations these scripts actually use, in display order: the schemes we
// know in the order above, then schemes we do not, then the integrator-only
// groups, then the tools that serve every integration — the general last, the
// way the unclassified domain is.
function integrationOrder(scripts) {
  const present = new Set(scripts.map(integrationOf));
  const schemes = [...present].filter((id) => id.startsWith(SCHEME)).map((id) => id.slice(SCHEME.length));
  return [
    ...INTEGRATION_IDS.filter((i) => schemes.includes(i)).map((i) => SCHEME + i),
    ...schemes.filter((i) => !INTEGRATION_IDS.includes(i)).sort().map((i) => SCHEME + i),
    ...[...present].filter((id) => id.startsWith(INTEGRATOR)).sort(),
    ...(present.has(ANY_INTEGRATION) ? [ANY_INTEGRATION] : []),
  ];
}

// A value every script in the group states, and states the same way — or null. A
// heading may only speak for a group when it speaks for all of it: two countries
// under one scheme means the rows keep their country column.
function agreedOn(scripts, field) {
  const first = scripts[0]?.[field];
  return first && scripts.every((s) => s[field] === first) ? first : null;
}

// A subsection's heading, and the frontmatter fields it has already said out
// loud. Both surfaces that group by integration read `covers` to drop a column
// rather than print it beside a heading that just gave the same answer.
function integrationGroup(id, scripts = []) {
  if (id === ANY_INTEGRATION) return { id, label: "Any integration", covers: ["integration"] };
  if (id.startsWith(INTEGRATOR)) {
    return { id, label: `${titleOf(id.slice(INTEGRATOR.length))} · any scheme`, covers: ["integration", "integrator"] };
  }
  const slug = id.slice(SCHEME.length);
  const parts = [INTEGRATIONS[slug]?.label ?? titleOf(slug)];
  const covers = ["integration"];
  const country = agreedOn(scripts, "country");
  if (country) {
    parts.push(country);
    covers.push("country");
  }
  const integrator = agreedOn(scripts, "integrator");
  if (integrator) {
    parts.push(integrator);
    covers.push("integrator");
  }
  return { id, label: parts.join(" · "), covers };
}

// Production before staging, then read-only before write, then by name. The
// environment no longer has a heading of its own, so it keeps the rows of a
// subsection in blocks instead.
function byEnvAccessName(a, b) {
  return ENVS.indexOf(a.env) - ENVS.indexOf(b.env) || a.access.localeCompare(b.access) || a.name.localeCompare(b.name);
}
