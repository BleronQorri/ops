...
function execRun(rec, exe, args, mode, fromEnvFile = {}) {
  return new Promise((resolve_) => {
    // Lowest to highest: the .env file, then what your shell exported, then the
    // -p parameters. A secret is only ever in the first of those — it is never
    // written to the record the way a parameter is.
    const env = { ...fromEnvFile, ...process.env, ...(rec.params || {}) };
...
function hintMisplacedFlags(args) {
  ...
  for (const flag of ["--no-log", "--no-secrets"]) {
    if (args.includes(flag)) {
      ui.eprint(cerr.faint(`  (note: ${flag} goes before the script name; this one went to the script)`));
    }
  }
}

async function doRun({ token, args, params, log, rerunOf, useSecrets = true }) {
  const { script, exe, label } = resolve(token);
  if (args[0] === "--") args = args.slice(1);

  // Before anything is recorded or spawned: the credentials this script declares.
  // Whatever is missing is asked for once and saved; whatever has expired is
  // asked for again rather than handed over dead. Skipping a prompt is fine — the
  // script then asks the way it always has.
  const declared = useSecrets ? script.secrets : [];
  const bag = useSecrets ? await secrets.forRun(declared, { label: script.name }) : { env: {} };
  const used = declared.filter((n) => bag.env[n] !== undefined);
  ...
  const rec = runs.create({
    ...
    cwd: process.cwd(),
    // Names only. The values live in the .env file and in the child's
    // environment, and in neither case in anything ops writes down.
    secrets: used,
    log: null,
    captured: "none",
  ...
  const { code, signal, failedToStart } = await execRun(rec, exe, args, mode, bag.env);
  if (failedToStart) {
    rec.status = "failed";
  ...
  // ops cannot tell a dead token from a bad argument — only the script knows why
  // it failed. So it names what it supplied and asks, and only when the run's own
  // output implicates the credential.
  if (used.length && credentialSuspect(rec, code)) {
    await secrets.offerReplacement(used, { label: `run ${rec.id} (${rec.label})` });
  }
  return code;
}

// Exit 1 is "the data is wrong" — a receipt that is not there is not a dead token —
// so a replacement is offered only when the credential is actually implicated:
// the run's own output says it was rejected, or nothing was captured and the exit
// code is the one a refused call produces.
const AUTH_RE = /\b(401|403|unauthoriz(ed|ation)|unauthorised|forbidden|invalid[ _-]?(token|credential|credentials|api[ _-]?key)|token (was )?(refused|rejected|expired)|expired token|authentication failed)\b/i;

function credentialSuspect(rec, code) {
  if (code === 0) return false;
  const log = readLog(rec);
  if (log !== null) return AUTH_RE.test(cleanLog(log));
  return code === 2;
}
...
function addRun(parent) {
  ...
  const run = parent
    ...
    .option("--no-secrets", "do not pass the saved secrets, and do not ask for a missing one")
    .passThroughOptions()
    .allowUnknownOption()
    .allowExcessArguments()
    .action(async (token, args, opts) => {
      process.exitCode = await doRun({ token, args, params: parseParams(opts.param || []), log: opts.log !== false, useSecrets: opts.secrets !== false });
    });
  gh(run, {
    usage: ["[flags] <script> [<script-args>...]"],
    examples: [
      { cmd: "ops task run force_retry_invoices --dry-run 123,456" },
      { cmd: "ops task run force_retry_invoices --help", note: "the script's own help, not this page" },
      { cmd: "ops task run -p INVOPOP_API_TOKEN=… check_invopop_suppliers --problems", note: "parameters reach the script as environment variables, and go before the script name" },
...
function register(program) {
  ...
  const rerun = task
    ...
    .action(async (id, args, opts) => {
      ...
      process.exitCode = await doRun({ token: rec.script, args: useArgs, params, log: opts.log !== false, rerunOf: rec.id });
    });
  gh(rerun, {
  ...
  const cancel = task
    ...
    .argument("<id>", "run id")
    .action(async (id) => {
      ui.eprint(await doCancel(requireRun(id)));
    });
  gh(cancel, { usage: ["<id>"], examples: [{ cmd: "ops task cancel 42" }] });
}

// SIGTERM, then SIGKILL if it is still there five seconds later. Returns the line
// to report and raises ProblemsError when there was nothing to stop; shared by
// `ops task cancel` and the runs pane of `ops interactive`.
async function doCancel(rec) {
  ...
  if (!runs.pidAlive(rec)) return `run ${rec.id} cancelled`;
  try {
    process.kill(rec.pid, "SIGKILL");
  } catch {
    /* it went away between the check and the signal */
  }
  return `run ${rec.id} did not stop on SIGTERM — killed`;
}
...
module.exports = { register, doRun, doCancel, credentialSuspect, parseParams, matchesParam, duration, human, ago, cleanLog, detail, readLog, statusMark, argsCell, scriptCell };
