...
function banner(rec, script) {
  const bits = [`${script.env} · ${script.access} · ${tierLabel(script)}`];
...
function hintMisplacedFlags(args) {
  ...
  if (i !== -1 && /^[A-Za-z_][A-Za-z0-9_]*=/.test(args[i + 1] || "")) {
    ui.eprint(cerr.faint("  (note: ops flags go before the script name — 'ops orion task run -p KEY=VALUE <script> …'; this one went to the script)"));
  }
...
async function doRun({ token, args, params, log, reserved, rerunOf }) {
  ...
  ui.eprint(
    `${cerr.faint("ops ▸")} run ${rec.id} ${statusMark(rec.status)} in ${human(rec.duration_ms)}` +
      ...
      (rec.log ? cerr.faint(`  · ops orion task logs ${rec.id}`) : "")
  );
  return code;
}
...
function detail(rec) {
  const rows = [
    ...
    ["Tier", rec.tier ? `${rec.tier} — ${cat.TIERS[rec.tier]?.label ?? ""}` : "—"],
...
function requireRun(id) {
  if (!/^\d+$/.test(String(id))) throw new UsageError(`invalid run id: ${id}`, "ids are numbers — see `ops orion task ls`");
...
function register(orion, { reserved }) {
  ...
  const task = orion
    .command("task")
    .summary("Run scripts and inspect past runs")
    .description("Run the Orion scripts and keep a record of every run, the way `houston task` does: each run gets an id, a status, the parameters it was given and a captured log, and stays queryable afterwards.\n\nRuns are recorded under $OPS_STATE_DIR (default ~/.local/state/ops/orion/runs); the newest 500 are kept.")
    .helpGroup("SCRIPT COMMANDS")
    .enablePositionalOptions();
  gh(task, {
    examples: [
      { cmd: "ops orion task run retry_invoices --dry-run 123,456" },
      { cmd: "ops orion task ls --script ri" },
      { cmd: "ops orion task get 42" },
      { cmd: "ops orion task rerun 42" },
    ],
  });
  ...
  gh(run, {
    ...
    examples: [
      { cmd: "ops orion task run retry_invoices --dry-run 123,456" },
      { cmd: "ops orion task run ri --help", note: "the script's own help (alias from its frontmatter)" },
      { cmd: "ops orion task run -p INVOPOP_API_TOKEN=… isc --problems", note: "parameters reach the script as environment variables, and go before the script name" },
      { cmd: "ops orion task run b2b_credit_notes/decode_payloads --ids 1 --yes", note: "a secondary entrypoint" },
      { cmd: "cd ~/reports && ops orion task run plugin_legal_entity_updates", note: "reports land in the current directory" },
    ],
  });
  ...
  const ls = task
    ...
    .action((opts) => {
      ...
      process.stdout.write(
        ui.table(
          list.map((r) => [String(r.id), statusMark(r.status), r.script, ago(r.started_at), human(r.duration_ms), argsCell(r)]),
  ...
  gh(ls, {
    usage: ["[flags]"],
    examples: [
      { cmd: "ops orion task ls" },
      { cmd: "ops orion task ls --script retry --status failed" },
      { cmd: "ops orion task ls --param PROVIDER_IDS=123", note: "matches one token of a comma-separated value" },
      { cmd: "ops orion task ls --since 7d --json | jq -r '.[] | [.id, .script, .status] | @tsv'" },
    ],
  });
  ...
  const get = task
    ...
    .action((id, opts) => {
      ...
      if (text) {
        ...
        process.stdout.write(`\n${ui.section(`LOG  ${c.faint(`(last ${shown.length} of ${lines.length} lines — ops orion task logs ${rec.id})`)}`, shown.map((l) => `  ${l}`).join("\n"))}\n`);
      }
      if (rec.status === "failed") throw new ProblemsError("");
    });
  gh(get, { usage: ["<id> [flags]"], examples: [{ cmd: "ops orion task get 42" }, { cmd: "ops orion task get 42 --json | jq .params" }] });
  ...
  gh(logs, { usage: ["<id> [flags]"], examples: [{ cmd: "ops orion task logs 42" }, { cmd: "ops orion task logs 42 --no-pager | grep ERROR" }] });
  ...
  gh(rerun, {
    ...
    examples: [{ cmd: "ops orion task rerun 42" }, { cmd: "ops orion task rerun 42 --dry-run 999", note: "same script, different arguments" }],
  });
  ...
  gh(cancel, { usage: ["<id>"], examples: [{ cmd: "ops orion task cancel 42" }] });
}
