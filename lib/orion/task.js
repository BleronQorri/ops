...
async function doRun({ token, args, params, log, rerunOf, useSecrets = true }) {
  ...
  // it failed. So it names what it supplied and asks, rather than assuming.
  if (code !== 0 && used.length) await secrets.offerReplacement(used, { label: `run ${rec.id} (${rec.label})` });
  return code;
}
...
module.exports = { register, doRun, doCancel, parseParams, matchesParam, duration, human, ago, cleanLog, detail, readLog, statusMark, argsCell, scriptCell };
