...
// Markdown -> terminal. Fenced code is indented and cyan; a pipe table becomes a
// real box table; a blockquote becomes an indented bar; inline code, bold, italics
// and links lose their markers and keep their text.
function inline(text) {
  return String(text)
    .replace(/!?\[([^\]]+)\]\([^)]+\)/g, "$1")
    .replace(/\*\*([^*]+)\*\*/g, (_, x) => c.bold(x))
    .replace(/(^|[\s(])[*_]([^*_\s][^*_]*)[*_](?=[\s.,;:)!?]|$)/g, (_, pre, x) => pre + x)
    .replace(/`([^`]+)`/g, (_, x) => c.sql(x));
}

// | a | b |  ->  cells. A separator row (---|---) is dropped.
...
function mdToTerm(lines, indent = "  ") {
  ...
  const flushTable = () => {
    ...
    while (i < lines.length && /^\s*\|/.test(lines[i])) {
      if (!isTableSeparator(lines[i])) rows.push(tableRow(lines[i]));
      i++;
    }
    if (!rows.length) return;
    const cols = rows.shift();
    const width = Math.max(40, ui.width() - indent.length);
    const rendered = ui.table(rows, cols, { tty: true, maxWidth: width });
  ...
  while (i < lines.length) {
    ...
    if (/^\s*>/.test(line)) {
      // a blockquote is a note: keep it, lose the marker
      const body = line.replace(/^\s*>\s?/, "");
      out.push(body.trim() === "" ? indent + c.faint("│") : `${indent}${c.faint("│")} ${inline(body)}`);
      i++;
      continue;
    }
    if (/^\s*(-{3,}|\*{3,}|_{3,})\s*$/.test(line)) {
      out.push(indent + c.faint("─".repeat(Math.max(10, Math.min(60, ui.width() - indent.length - 2)))));
      i++;
      continue;
    }
    out.push(line.trim() === "" ? "" : indent + inline(line));
    i++;
  }
...
function metaBlock(script) {
  ...
  const rows = [
    ["Domain", script.domain ? cat.DOMAINS[script.domain]?.label ?? script.domain : c.faint("(unclassified)")],
  ];
  if (script.country) rows.push(["Country", script.country]);
  if (script.integration || script.integrator) {
    rows.push(["Integration", [script.integration, script.integrator && `via ${script.integrator}`].filter(Boolean).join("  ·  ")]);
  }
  rows.push(
    ["Env", script.env],
  ...
  if (script.retired_reason) rows.push(["Retired", script.retired_reason + (script.retired_on ? `  (${script.retired_on})` : "")]);
  ...
  if (script.secrets.length) {
    ...
    rows.push([
      ...
      script.secrets
        .map((n) => {
          if (process.env[n] !== undefined) return `${n} ${c.faint("(exported)")}`;
          if (saved[n] === undefined) return `${n} ${c.faint("(ops will ask)")}`;
          if (secrets.expired(saved[n])) return `${n} ${c.warn("(saved, expired)")}`;
          return `${n} ${c.ok("(saved)")}`;
        })
        .join(", "),
    ]);
...
function register(script) {
  ...
  gh(cmd, {
    ...
    examples: [
      ...
      { cmd: "ops orion script view force_retry_invoices --raw", note: "the whole AGENTS.md" },
      { cmd: "cd \"$(ops orion script view match_credit_notes_to_invoices --path)\"", note: "jump to the directory" },
    ],
  });
}
