...
function metaBlock(script) {
  const tier = script.tier ? `${script.tier} — ${cat.TIERS[script.tier]?.label ?? ""}` : c.faint("(no frontmatter yet)");
  const rows = [
    ["Env", script.env],
    ["Access", script.access],
    ["Tier", tier],
    ["Status", script.legacy ? c.warn("unannotated") : script.status === "active" ? "active" : c.warn(script.status.toUpperCase())],
  ];
  ...
  if (script.env_vars.length) rows.push(["Env vars", script.env_vars.join(", ")]);
...
function renderView(script, root) {
  ...
  if (!script.docOnly && script.exe) {
    ...
    else if (script.lang === "exs" && script.help_flag) parts.push(ui.section("SCRIPT HELP", `  Flags: ${c.cmd(`ops orion script run ${script.name} -- --help`)}  ${c.faint("(Elixir, slow start — not run here)")}`));
    else {
  ...
  if (script.examples.length) {
    parts.push(ui.section("EXAMPLES", ui.examples(script.examples.map((ex) => ({ note: ex.note, cmd: `ops orion script run ${script.name}${ex.args ? " " + ex.args : ""}` })))));
  }
