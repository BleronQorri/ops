...
function register(program) {
  ...
  gh(orion, {
    examples: [{ cmd: "ops orion script list" }, { cmd: "ops orion script view retry_invoices" }, { cmd: "ops orion docs check" }],
  });
  ...
  // `ops run` and `ops task` sit at the root: running a script is what this tool
  // is for, and it should not be three words deep.
  require("./task").register(program, ctx);
