...
function register(script) {
  ...
  gh(cmd, {
    usage: ["<name> --lang {js | exs} --env {production | staging} --access {read-only | write} [flags]"],
    examples: [
      { cmd: "ops orion script new requeue_documents --lang js --env production --access write --summary 'Requeue rejected documents for resend'" },
      { cmd: "ops orion script new seed_provider --lang exs --env staging --access write --tier staging-destructive" },
    ],
  });
}
