...
function register(script) {
  ...
  const cmd = script
    .command("pick")
    .summary("Interactive picker: arrow keys, Enter to run, v to view (TTY only)")
    .description("Browse the catalogue interactively and pick one script. Type to filter by name or summary; ↑/↓ (or j/k) move; Enter runs it; v opens the script's page; q quits.\n\nIt ends as soon as it has picked something. `ops interactive` keeps the screen open and adds the run history.")
    .action(async () => {
      ...
      const choice = await screen.select({
        ...
        hint: "type to filter · ↑↓ move · Enter run · v view · q quit",
        mode: "type",
        keys: { v: "view" },
