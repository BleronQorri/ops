...
const GROUPING_FIELDS = ["domain", "env"];
// Columns that only some domains have anything to say in. A section where every
// one of them is empty drops them rather than printing a column of dashes.
...
function register(script) {
  ...
  const cmd = script
    ...
    .option("-i, --integration <name>", "filter by integration (verifactu, zatca, smart_receipts); repeatable", collect, [])
    ...
    .option("--flat", "one table for every environment instead of a section each")
    ...
    .action((opts) => {
      ...
      // Piped: one flat table, every field, no headings — the ENV column carries the
      // environment so nothing is lost to a script.
      ...
      // columns: what a script is about first, then production and staging, which
      // stay separate worlds inside it.
      ...
      for (const domain of cat.domainOrder(rows)) {
        ...
        // Decided once per domain, so the production and staging tables inside it
        // still line up with each other.
        const cols = grouped.filter((f) => !OPTIONAL_FIELDS.includes(f) || inDomain.some((r) => r[f] !== null && r[f] !== undefined));
        const blocks = [`${ui.c.header(cat.domainLabel(domain).toUpperCase())}  ${count(inDomain.length)}`];
        for (const env of cat.ENVS) {
          const group = inDomain.filter((r) => r.env === env);
          if (!group.length) continue;
          group.sort((a, b) => a.access.localeCompare(b.access) || a.name.localeCompare(b.name));
          blocks.push(`  ${ui.c.header(env.toUpperCase())}  ${count(group.length)}`);
          const body = ui.table(group.map((r) => cols.map((f) => cell(r, f))), cols.map((f) => f.toUpperCase()), { maxWidth: ui.width() - 2 }).replace(/\n$/, "");
  ...
  gh(cmd, {
    ...
    examples: [
      ...
      { cmd: "ops orion script list --integration verifactu --integration zatca" },
