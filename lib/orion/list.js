...
const FIELDS = ["name", "env", "access", "tier", "status", "lang", "summary", "aliases", "entrypoint", "dir", "also", "reports", "related", "blocked_on", "retired_reason", "retired_on"];
...
function register(script, { reserved }) {
  ...
  const cmd = script
    ...
    .action((opts) => {
      ...
      // On a terminal: production and staging are separate worlds, so they get
      // separate tables. ENV becomes the heading and leaves the columns.
      const grouped = fields.filter((f) => f !== "env");
      const sections = [];
      for (const env of cat.ENVS) {
        const group = rows.filter((r) => r.env === env);
        if (!group.length) continue;
        group.sort((a, b) => a.access.localeCompare(b.access) || a.name.localeCompare(b.name));
        const heading = `${ui.c.header(env.toUpperCase())}  ${ui.c.faint(`${group.length} script${group.length === 1 ? "" : "s"}`)}`;
        const body = ui.table(group.map((r) => grouped.map((f) => cell(r, f))), grouped.map((f) => f.toUpperCase())).replace(/\n$/, "");
