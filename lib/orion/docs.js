...
function register(orion, { reserved }) {
  ...
  const docs = orion
    ...
    .description("Keep the scripts repo's documentation honest. `check` validates every AGENTS.md frontmatter, entrypoint and the generated regions; `sync` rewrites the generated regions from frontmatter.")
  ...
  const sync = docs
    ...
    .description("Rewrite only the text between `<!-- ops:begin <id> -->` and `<!-- ops:end <id> -->` (or `# ops:begin` in .gitignore) in the scripts repo's AGENTS.md, README.md and .gitignore. Everything outside the markers is preserved byte for byte. A file with unbalanced markers is refused; missing marker pairs are reported and never inserted.")
