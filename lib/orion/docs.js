...
function catalogueTable(scripts) {
  // The For column is empty for a tool that serves every country, which is a fact
  // about the tool and not a gap in the table.
  const lines = ["| Script | For | Tier | Run | What it does |", "|--------|-----|------|-----|--------------|"];
  for (const s of scripts) {
    const forCell = [s.country, s.integration].filter(Boolean).join(" · ") || "—";
    lines.push(`| [${s.name}](${s.name}/)${statusMark(s)} | ${forCell} | ${s.tier ?? "?"} | ${runCell(s)} | ${mdCell(s.summary)}${s.blocked_on ? ` **Blocked on:** ${mdCell(s.blocked_on)}` : ""} |`);
  }
  return lines.join("\n");
}
...
function register(orion) {
  ...
  const check = docs
    ...
    .description("Exit 0 when every script directory has valid frontmatter, its entrypoint is executable with the right shebang and answers `--help`, and the generated regions in AGENTS.md / .gitignore match what `sync` would write. Warnings do not fail the check.")
    .option("--probe-exs", "also run `--help` on Elixir scripts (slow: Mix.install)")
    .option("--json", "output problems as JSON")
    .action((opts) => {
      const { catalogue, problems } = runCheck({ probeExs: opts.probeExs });
