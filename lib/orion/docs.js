...
// `ops orion docs sync`  — regenerate the marker-delimited regions in the
//                          scripts repo's AGENTS.md, README.md and .gitignore.
...
function runCell(s) {
  ...
  const parts = [`\`ops orion script run ${s.name}\``, ...s.aliases.map((a) => `\`${a}\``)];
  return parts.join(" · ");
}
...
function catalogueTable(scripts) {
  ...
  for (const s of scripts) {
    lines.push(`| [${s.name}](${s.rel}/)${statusMark(s)} | ${s.tier ?? "?"} | ${runCell(s)} | ${mdCell(s.summary)}${s.blocked_on ? ` **Blocked on:** ${mdCell(s.blocked_on)}` : ""} |`);
  }
  return lines.join("\n");
}
...
function readmeList(scripts) {
  const lines = [];
  for (const env of cat.ENVS) {
    for (const access of cat.ACCESSES) {
      const group = scripts.filter((s) => s.env === env && s.access === access);
      if (!group.length) continue;
      lines.push(`### \`${env}/${access}\``, "");
      for (const s of group) {
        const mark = s.status === "deprecated" ? " ⚠️ _deprecated_" : s.status === "blocked" ? " ⛔ _blocked_" : s.status === "runbook" ? " 📄 _runbook, no script_" : "";
        lines.push(`- **${s.name}**${mark} — ${s.summary}${s.tier ? ` _(${s.tier})_` : ""}`);
      }
      lines.push("");
    }
  }
...
function generateRegions(catalogue) {
  const S = catalogue.scripts;
  const regions = { "AGENTS.md": {}, "README.md": {}, ".gitignore": {} };
  ...
  regions["README.md"].catalogue = readmeList(S);
  regions[".gitignore"].reports = reportsGlobs(S);
  return regions;
}
...
function runCheck({ reserved, probeExs, json }) {
  ...
  for (const s of catalogue.scripts) {
    if (s.legacy || s.docOnly || !s.exe || !s.help_flag || !cat.isExecutable(s.exe)) continue;
...
function register(orion, { reserved }) {
  ...
  const docs = orion
    ...
    .description("Keep orion/'s documentation honest. `check` validates every AGENTS.md frontmatter, entrypoint and the generated regions; `sync` rewrites the generated regions from frontmatter.")
  ...
  const check = docs
    ...
    .description("Exit 0 when every script directory has valid frontmatter, its entrypoint is executable with the right shebang and answers `--help`, aliases do not collide, and the generated regions in AGENTS.md / README.md / .gitignore match what `sync` would write. Warnings do not fail the check.")
  ...
  const sync = docs
    ...
    .description("Rewrite only the text between `<!-- ops:begin <id> -->` and `<!-- ops:end <id> -->` (or `# ops:begin` in .gitignore) in orion/'s AGENTS.md, README.md and .gitignore. Everything outside the markers is preserved byte for byte. A file with unbalanced markers is refused; missing marker pairs are reported and never inserted.")
