...
// Repainting, not erasing and redrawing. Moving the cursor up and erasing to the
// end of the screen before writing the new frame leaves the terminal blank for
// however long the two writes take, and at one keystroke per row that reads as a
// flicker. Instead every frame goes out as ONE write that overwrites the old text
// in place — each line clears only its own tail (\x1b[K) — and the screen is only
// erased below when the new frame is shorter than the old one.
...
// The cursor is hidden for the life of a frame, so it has to come back even if
// the process ends somewhere a frame never finished — an exception in the loop,
// or a signal. One handler for the whole module, armed only once.
...
function showCursor() {
  ...
  try {
    ...
    /* the stream is already gone: nothing left to restore it on */
  }
}
...
// groups(filter) -> [{ section, heading, note, items: [{ value, render(width, selected) }] }]
//   section  an outer heading printed once, when it changes between groups
//   heading  the group's own heading, printed for every group
//
// footer is a string, or a function of the highlighted value — the place to say
// what the keys would do to the row the cursor is on, which is the moment anyone
...
function select({ title, hint, groups, keys = {}, mode = "type", empty = "no match", footer = null, filter = "", cursor = 0 }) {
  return new Promise((resolve) => {
    ...
    let drawn = 0;
    let painted = null; // the last frame's text, to skip a repaint that changes nothing
    let hidden = false;
    let detach = null;

    // groups() re-filters and re-measures every call, and a keypress used to ask
    // for it three times. It depends only on the filter, so one result per filter
    // is enough.
    let cache = null;
    const build = () => {
      if (cache && cache.key === filter) return cache.value;
      const lines = [];
      const values = [];
      let section = null;
      for (const g of groups(filter)) {
        ...
        if (g.section && g.section !== section) {
          if (lines.length) lines.push({ blank: true });
          lines.push({ section: g.section, note: g.sectionNote });
          section = g.section;
        }
        // Every group gets air above it, not just the first of a section.
        else if (g.heading && lines.length) lines.push({ blank: true });
      ...
      cache = { key: filter, value: { lines, values } };
      return cache.value;
    };

    const footerFor = (value) => {
      const text = typeof footer === "function" ? footer(value) : footer;
      return text || null;
    };
    ...
    const draw = () => {
      ...
      if (start > 0 && win.length && !win[0].heading && !win[0].section) {
        const above = lines.slice(0, start).reverse().find((l) => l.heading || l.section);
        if (above) {
          win.unshift({ ...above, note: "continued" });
          win.pop();
        }
      }
      ...
      for (const l of win) {
        ...
        if (l.section) {
          out.push(`${c.header(l.section)}${l.note ? `  ${c.sql(l.note)}` : ""}`);
          continue;
        }
        if (l.heading) {
          out.push(`  ${c.bold(l.heading)}${l.note ? `  ${c.sql(l.note)}` : ""}`);
          continue;
        }
      ...
      const foot = footerFor(values[cursor]);
      if (foot) out.push(`  ${foot}`);

      const text = out.join("\n");
      if (text === painted) return; // the keystroke changed nothing on screen
      let buf = "";
      if (!hidden) {
        buf += "\x1b[?25l"; // the cursor stays hidden for the life of the frame
        hidden = true;
        cursorHidden = true;
      }
      if (drawn) buf += `\x1b[${drawn}A\r`;
      buf += out.map((l) => `${l}\x1b[K`).join("\n") + "\n";
      if (drawn > out.length) buf += "\x1b[J"; // the frame shrank: clear what is left below
      stdout.write(buf);
      drawn = out.length;
      painted = text;
    };
    ...
    const finish = (result) => {
      if (detach) detach();
      // Erase the frame and give the cursor back, in one write: a script's own
      // prompts run next and they need it.
      stdout.write((drawn ? `\x1b[${drawn}A\r\x1b[J` : "") + (hidden ? "\x1b[?25h" : ""));
      hidden = false;
      cursorHidden = false;
      drawn = 0;
      painted = null;
    ...
    const onKey = (str, key) => {
      ...
      switch (key.name) {
        ...
        case "escape":
          ...
          return finish(null);
        case "return":
          // Enter closes the filter AND acts on the row: filtering is how you
          // reach a row, so swallowing the keystroke that arrives the moment you
          // have reached it makes the key look broken. The filter text stays —
    ...
    const detachKeys = onKeys(stdin, onKey);
    detach = () => {
