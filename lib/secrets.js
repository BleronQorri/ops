...
//
// So does a JWT whose `exp` is zero, negative, or long before anyone was issuing
// JWTs: `exp: 0` is how "this one does not expire" is written, and reading it as
// the first second of 1970 would withhold a working token forever. This check
// exists to catch a token that has died, and a false positive costs more than a
// missed one — the run just asks, the way it always did.
const EXPIRY_FLOOR = Date.UTC(2000, 0, 1);
...
function jwtExpiry(value) {
  ...
  try {
    ...
    if (typeof claims.exp !== "number" || claims.exp <= 0) return null;
    const d = new Date(claims.exp * 1000);
    if (isNaN(d.getTime()) || d.getTime() < EXPIRY_FLOOR) return null;
    return d;
  } catch {
    return null;
  }
...
// `savedAt` is the second belt: a token cannot have expired before the day it was
// saved, so if it claims to, the claim is not one to act on.
function expired(value, now = Date.now(), savedOn = null) {
  const exp = jwtExpiry(value);
  if (exp === null) return false;
  if (savedOn && exp.getTime() < savedOn.getTime()) return false;
...
async function forRun(declared = [], { prompt = true, label = "" } = {}) {
  ...
  const wanted = (name) => {
    ...
    if (expired(saved, Date.now(), savedAt(name))) return "expired";
    return null;
  };
  ...
  for (const [name, value] of Object.entries(file)) if (expired(value, Date.now(), savedAt(name))) delete file[name];
